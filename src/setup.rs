...
pub const AGENTS: [&str; 6] = ["claude", "codex", "grok", "agy", "opencode", "pi"];
...
pub fn run(home: &Path, agent: &str, remove: bool) -> Result<()> {
    let agents: Vec<&str> = if agent == "all" {
        ...
        return Err(anyhow!(
            "unknown agent {agent}: use claude | codex | grok | agy | opencode | pi | all"
        ));
    };
    ...
    for a in agents {
        ...
        let mcp = match a {
            ...
            .to_string(),
            "pi" => continue, // Pi uses the extension's native CLI tools, not MCP.
            _ => unreachable!(),
        };
