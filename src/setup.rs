...
const PI_MARKER: &str = "// Generated by oboete setup pi.\n";
...
fn wire(a: &str, cmd: &HookCommand, remove: bool) -> Result<()> {
    ...
    let kind = if a == "pi" { "extension" } else { "hooks" };
    if files.is_empty() {
        let state = if remove {
            "nothing to remove"
        } else {
        ...
        println!("{a}: {kind}: {state}");
    } else {
        println!("{a}: {kind} {verb} {}", files.join(", "));
    }
    let mcp = match a {
...
impl HookCommand {
    ...
    fn cursor_line(&self, event: &str, windows: bool) -> Result<String> {
        if !windows {
            return Ok(self.line("cursor", event));
        }
        // Keep agy's Windows path refusal; PowerShell's double quotes also interpolate $ and `.
        for path in std::iter::once(&self.exe).chain(self.home.iter()) {
            anyhow::ensure!(
                !path
                    .chars()
                    .any(|c| c.is_whitespace() || "&|<>()^%!\"'`$".contains(c)),
                "cursor on Windows needs a space-free executable and --home path without PowerShell metacharacters (got {path})"
            );
        }
        let home = self
            .home
            ...
            .map(|h| format!(" --home \"{h}\""))
            .unwrap_or_default();
        Ok(format!("& \"{}\"{home} hook cursor {event}", self.exe))
    }
}
...
fn is_our_handler(h: &Value) -> bool {
    ...
    let words = if words.first().is_some_and(|w| w == "&") {
        &words[1..]
    } else {
        &words
    };
...
/// Inverse of our command builders: whitespace-separated words, POSIX single-quoted segments
/// (with `'\''` for a literal quote), and PowerShell double-quoted paths kept whole.
...
fn pi_extension(cmd: &HookCommand) -> Result<String> {
    let args = match &cmd.home {
        Some(home) => vec!["--home", home.as_str()],
        None => Vec::new(),
    };
    Ok(format!(
        "{PI_MARKER}const OBOETE_BIN = {};\nconst OBOETE_ARGS = {};\n{}",
        serde_json::to_string(&cmd.exe)?,
...
fn pi_files(dir: &Path, cmd: &HookCommand, remove: bool) -> Result<Vec<String>> {
    let file = dir.join("extensions/oboete.ts");
    let old = read_text(&file)?;
    if remove {
        if !old.starts_with(PI_MARKER) {
            return Ok(Vec::new());
        }
        refuse_read_only(&file)?;
        std::fs::remove_file(&file).with_context(|| format!("remove {}", file.display()))?;
    } else {
        let text = pi_extension(cmd)?;
        if old == text {
            return Ok(Vec::new());
        }
        let staged = stage(&file, &text)?;
        backup_once(&file)?;
        staged.commit()?;
    }
    Ok(vec![file.display().to_string()])
...
fn pi_extension_status(file: &Path, cmd: &HookCommand) -> String {
    match std::fs::read_to_string(file) {
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
            "extension missing (run `oboete setup pi`)".into()
        }
        Err(e) => format!("unreadable: {e}"),
        Ok(text) if pi_extension(cmd).is_ok_and(|wanted| text == wanted) => {
            "extension wired (current binary and home)".into()
        }
        Ok(_) => "extension stale (rerun `oboete setup pi`)".into(),
    }
}

fn cursor_dir_from(config_dir: Option<&Path>, xdg: Option<&Path>, home: &Path) -> PathBuf {
    // The installed Cursor bundle treats empty or whitespace-only environment values as unset.
    let nonempty = |p: &&Path| !p.as_os_str().to_string_lossy().trim().is_empty();
    if let Some(dir) = config_dir.filter(nonempty) {
        dir.to_path_buf()
    } else if let Some(dir) = xdg.filter(nonempty) {
        dir.join("cursor")
    } else {
        home.join(".cursor")
    }
...
fn cursor_available(dir: &Path, cursor_agent: bool, agent: bool) -> bool {
    dir.is_dir() || cursor_agent || agent
}

fn cursor_hook_spec(cmd: &HookCommand, windows: bool) -> Result<Value> {
    let mut hooks = serde_json::Map::new();
    for (step, event) in CURSOR_EVENTS {
        hooks.insert(
            step.to_string(),
            json!([{"command":cmd.cursor_line(event, windows)?,"timeout":10}]),
        );
    }
    Ok(Value::Object(hooks))
}

fn cursor_read_json(file: &Path) -> Result<Value> {
    if !file.exists() {
        return Ok(json!({}));
    }
    let text = std::fs::read_to_string(file).with_context(|| format!("read {}", file.display()))?;
    let root: Value =
        serde_json::from_str(&text).with_context(|| format!("parse {}", file.display()))?;
    anyhow::ensure!(root.is_object(), "{} is not a JSON object", file.display());
    Ok(root)
}

fn cursor_merge_hooks(root: &mut Value, spec: &Value, remove: bool) -> Result<()> {
    let obj = root.as_object_mut().expect("checked JSON object");
    if !remove {
    ...
    for (step, entries) in hooks.iter_mut() {
        let array = entries
            .as_array_mut()
            .ok_or_else(|| anyhow!("hooks.{step} must be an array"))?;
        let had_ours = array.iter().any(is_our_handler);
        array.retain(|v| !is_our_handler(v));
        if had_ours && array.is_empty() {
            emptied_by_us.push(step.clone());
        }
    }
    // Keep already-empty arrays; remove a key only when its last entry was ours.
    if !remove {
        for step in emptied_by_us {
            if spec.get(&step).is_none() {
                hooks.remove(&step);
            }
        }
        for (step, entries) in spec.as_object().expect("spec object") {
            let array = hooks
                .entry(step)
                .or_insert_with(|| json!([]))
                .as_array_mut()
                .ok_or_else(|| anyhow!("hooks.{step} must be an array"))?;
            array.extend(entries.as_array().expect("spec array").iter().cloned());
        }
    } else {
        for step in emptied_by_us {
            hooks.remove(&step);
...
fn cursor_merge_mcp(root: &mut Value, cmd: &HookCommand, remove: bool) -> Result<()> {
    let obj = root.as_object_mut().expect("checked JSON object");
    ...
    if remove {
        ...
        let entry = servers.entry(MCP_NAME).or_insert_with(|| json!({}));
        anyhow::ensure!(entry.is_object(), "mcpServers.oboete must be a JSON object");
        entry["command"] = json!(cmd.exe);
        entry["args"] = json!(cmd.mcp_args());
    }
    Ok(())
...
/// Both valid changes are staged before either is replaced. JSONC and malformed files are left
/// byte-for-byte intact, with the exact entry to add printed for manual editing.
fn cursor_files(dir: &Path, cmd: &HookCommand, remove: bool, windows: bool) -> Result<Vec<String>> {
    let hooks = dir.join("hooks.json");
    let mcp = dir.join("mcp.json");
    let spec = if remove {
        json!({})
    } else {
        cursor_hook_spec(cmd, windows)?
    };
    let entry = json!({"command":cmd.exe,"args":cmd.mcp_args()});
    let mut edits = Vec::new();
    for file in [&hooks, &mcp] {
        if remove && !file.exists() {
            continue;
        }
        let mut root = match cursor_read_json(file) {
            Ok(v) => v,
            Err(e) => {
                let manual = if file == &hooks {
                    json!({"version":1,"hooks":spec})
                } else {
                    json!({"mcpServers":{"oboete":entry}})
                };
                eprintln!(
                    "cursor: {} is not plain JSON ({e:#}); left untouched. {}: {}",
                    file.display(),
                    if remove {
                        "remove oboete entries manually"
                    } else {
        ...
        let old = root.clone();
        let merged = if file == &hooks {
            cursor_merge_hooks(&mut root, &spec, remove)
        } else {
            cursor_merge_mcp(&mut root, cmd, remove)
        };
        if let Err(e) = merged {
            eprintln!(
                "cursor: {} left untouched ({e:#}); edit manually: {}",
                file.display(),
                if file == &hooks {
                    serde_json::to_string_pretty(&json!({"version":1,"hooks":spec}))?
                } else {
                    serde_json::to_string_pretty(&json!({"mcpServers":{"oboete":entry}}))?
                }
            );
            continue;
        }
        if root != old {
            edits.push((file, json_text(&root)?));
        }
    }
    let mut staged = Vec::new();
    for (file, text) in &edits {
        staged.push((file, stage(file, text)?));
    }
    for (file, _) in &staged {
        backup_once(file)?;
    }
    let mut changed = Vec::new();
    for (file, edit) in staged {
        edit.commit()?;
        changed.push(file.display().to_string());
    }
    Ok(changed)
...
fn cursor_hooks_status(file: &Path, cmd: &HookCommand, windows: bool) -> String {
    let root = match cursor_read_json(file) {
        Ok(v) => v,
        Err(e) => return format!("unreadable: {e}"),
    };
    let mut correct = [false; CURSOR_EVENTS.len()];
    let mut stale = 0;
    for (step, entries) in root["hooks"].as_object().into_iter().flat_map(|h| h.iter()) {
        for entry in entries
            .as_array()
            .into_iter()
            .flatten()
            .filter(|v| is_our_handler(v))
        {
            let expected = CURSOR_EVENTS.iter().position(|(name, _)| name == step);
            let Some(index) = expected else {
                stale += 1;
                continue;
            };
            let command = match cmd.cursor_line(CURSOR_EVENTS[index].1, windows) {
                Ok(s) => s,
                Err(e) => return format!("cannot run: {e}"),
            };
            if entry["command"] == command && entry["timeout"] == 10 && !correct[index] {
                correct[index] = true;
            } else {
                stale += 1;
            }
    ...
    let count = correct.iter().filter(|v| **v).count();
    if stale > 0 {
        format!("stale: {count}/7 hooks wired, {stale} differ (rerun `oboete setup cursor`)")
    } else if count == 7 {
        "7 hooks wired".into()
    } else if count == 0 {
        "not wired (run `oboete setup cursor`)".into()
    } else {
        format!("partly wired: {count}/7 hooks")
    }
}
...
pub fn doctor(home: &Path) -> Result<()> {
    ...
    println!(
        "  pi      {}",
        pi_extension_status(&pi_dir().join("extensions/oboete.ts"), &want)
    );
    println!(
        "  cursor  {}",
        cursor_hooks_status(&cursor_dir().join("hooks.json"), &want, cfg!(windows))
    );
...
mod tests {
    ...
    #[test]
    fn pi_generated_extension_matches_event_and_tool_contract() {
        let dir = std::env::temp_dir().join(format!("oboete-pi-node-{}", std::process::id()));
        for home in [None, Some("C:\\Memory Store\\\"quoted\" 'home'".into())] {
            let cmd = HookCommand {
                exe: "C:\\Program Files\\oboete \"quoted\" 'x'.exe".into(),
                home,
            };
            pi_files(&dir, &cmd, false).unwrap();
            let args = match &cmd.home {
                Some(home) => vec!["--home", home.as_str()],
                None => Vec::new(),
            };
            let result = std::process::Command::new("node")
                ...
                .arg(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/testdata/pi/test.mjs"))
                .arg(dir.join("extensions/oboete.ts"))
                .arg(&cmd.exe)
            ...
            let output = match result {
                Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                    eprintln!("Pi extension harness skipped: node is not installed");
                    return;
                }
                other => other.unwrap(),
            };
    ...
    #[test]
    fn pi_setup_round_trip_preserves_other_extensions_and_checks_home() {
        assert!(AGENTS.contains(&"pi"));
        let dir = std::env::temp_dir().join(format!("oboete-pi-setup-{}", std::process::id()));
        let file = dir.join("extensions/oboete.ts");
        let other = dir.join("extensions/git-ai.ts");
        let cmd = HookCommand {
            exe: "C:\\Program Files\\oboete \"quoted\" 'x'.exe".into(),
            home: Some("C:\\Memory Store\\\"quoted\" 'home'".into()),
        };
        assert!(pi_files(&dir, &cmd, true).unwrap().is_empty());
        assert!(!dir.exists());
        assert!(pi_extension_status(&file, &cmd).contains("missing"));
        std::fs::create_dir_all(file.parent().unwrap()).unwrap();
        std::fs::write(&other, "export default function gitAi() {}\n").unwrap();
        // An unmarked extension is not ours to remove. Installation preserves a backup.
        let original = "// local extension\n";
        std::fs::write(&file, original).unwrap();
        assert!(pi_files(&dir, &cmd, true).unwrap().is_empty());
        assert_eq!(read_text(&file).unwrap(), original);
        assert!(pi_extension_status(&file, &cmd).contains("stale"));
        assert_eq!(pi_files(&dir, &cmd, false).unwrap().len(), 1);
        let generated = read_text(&file).unwrap();
        let constants: Vec<Value> = generated
            .lines()
            .filter_map(|line| line.strip_prefix("const OBOETE_"))
            .map(|line| {
                serde_json::from_str(line.split_once(" = ").unwrap().1.trim_end_matches(';'))
                    .unwrap()
            })
            .collect();
        assert_eq!(constants, [json!(cmd.exe), json!(["--home", cmd.home])]);
        assert_eq!(
            pi_extension_status(&file, &cmd),
            "extension wired (current binary and home)"
        );
        assert!(pi_files(&dir, &cmd, false).unwrap().is_empty());
        let other_home = HookCommand {
            exe: cmd.exe.clone(),
            home: None,
        };
        assert!(pi_extension_status(&file, &other_home).contains("stale"));
        let other_exe = HookCommand {
            exe: "/new/oboete".into(),
            home: cmd.home.clone(),
        };
        assert!(pi_extension_status(&file, &other_exe).contains("stale"));
        pi_files(&dir, &other_home, false).unwrap();
        assert!(
            read_text(&file)
                .unwrap()
                .contains("const OBOETE_ARGS = [];")
        );
        let backup = file.with_file_name("oboete.ts.oboete.bak");
        assert_eq!(read_text(&backup).unwrap(), original);
        assert_eq!(pi_files(&dir, &cmd, true).unwrap().len(), 1);
        assert!(!file.exists());
        assert_eq!(
            read_text(&other).unwrap(),
            "export default function gitAi() {}\n"
        );
        assert_eq!(read_text(&backup).unwrap(), original);
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    #[test]
    fn cursor_removes_owned_commands_using_the_shared_ownership_rule() {
        let own = json!({"command":"/old/oboete --home /old-home hook claude Stop"});
        assert!(has_ours(&json!({"hooks":[own]})));
        let other = json!({"command":"echo 'oboete hook cursor Stop'"});
        let mut root = json!({"version":1,"hooks":{"afterAgentResponse":[own,other],"custom":[]}});
        cursor_merge_hooks(&mut root, &json!({}), true).unwrap();
        assert_eq!(
            root,
            json!({"version":1,"hooks":{"afterAgentResponse":[other],"custom":[]}})
        );
    }

    #[test]
    fn cursor_empty_config_environment_values_fall_back_like_the_bundle() {
        let home = Path::new("/home/test");
        assert_eq!(
            cursor_dir_from(Some(Path::new("")), Some(Path::new("/xdg")), home),
            Path::new("/xdg/cursor")
        );
        assert_eq!(
            cursor_dir_from(Some(Path::new(" \t")), Some(Path::new("")), home),
            Path::new("/home/test/.cursor")
        );
    }

    #[test]
    fn cursor_round_trip_preserves_other_hooks_and_servers() {
        let dir = std::env::temp_dir().join(format!("oboete-cursor-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let hooks = dir.join("hooks.json");
        let mcp = dir.join("mcp.json");
        let original_hooks = json!({"version":2,"other":"keep","hooks":{
            "preToolUse":[{"command":"git-ai pre"}],
            "postToolUse":[{"command":"git-ai post","timeout":3}],
            "sessionEnd":[], "custom":[]
        }});
        let original_mcp = json!({"other":"keep","mcpServers":{
            "other":{"command":"x"},
            "oboete":{"command":"/old/oboete","args":["mcp"],"env":{"KEEP":"1"},"disabled":true}
        }});
        write_json(&hooks, &original_hooks).unwrap();
        write_json(&mcp, &original_mcp).unwrap();
        let original_hook_bytes = std::fs::read(&hooks).unwrap();
        let original_mcp_bytes = std::fs::read(&mcp).unwrap();
        let cmd = HookCommand {
            exe: "/x/oboete".into(),
            home: Some("/h".into()),
        };
        ...
        assert_eq!(h["other"], "keep");
        assert_eq!(
            h["hooks"]["preToolUse"],
            original_hooks["hooks"]["preToolUse"]
        );
        assert_eq!(
            h["hooks"]["postToolUse"][0],
            original_hooks["hooks"]["postToolUse"][0]
        );
        assert_eq!(h["hooks"]["sessionEnd"].as_array().unwrap().len(), 1);
        assert_eq!(h["hooks"]["custom"], json!([]));
        assert_eq!(cursor_hooks_status(&hooks, &cmd, false), "7 hooks wired");
        assert_eq!(
            m["mcpServers"]["other"],
            original_mcp["mcpServers"]["other"]
        );
        ...
        cursor_files(&dir, &cmd, false, false).unwrap();
        assert_eq!(std::fs::read(&hooks).unwrap(), first_h);
        assert_eq!(std::fs::read(&mcp).unwrap(), first_m);
        cursor_files(&dir, &cmd, true, false).unwrap();
        let mut expected_hooks = original_hooks;
        expected_hooks["hooks"]
            .as_object_mut()
            .unwrap()
            .remove("sessionEnd");
        assert_eq!(read_json_object(&hooks).unwrap(), expected_hooks);
        let mut expected_mcp = original_mcp;
        expected_mcp["mcpServers"]
            .as_object_mut()
            .unwrap()
            .remove("oboete");
        assert_eq!(read_json_object(&mcp).unwrap(), expected_mcp);
        assert_eq!(
            cursor_hooks_status(&hooks, &cmd, false),
            "not wired (run `oboete setup cursor`)"
        );
        assert_eq!(
            std::fs::read(hooks.with_file_name("hooks.json.oboete.bak")).unwrap(),
            original_hook_bytes
        );
        assert_eq!(
            std::fs::read(mcp.with_file_name("mcp.json.oboete.bak")).unwrap(),
            original_mcp_bytes
        );
        std::fs::remove_dir_all(dir).unwrap();
    }
    ...
    #[test]
    fn cursor_bad_json_leaves_each_file_untouched_independently() {
        let dir = std::env::temp_dir().join(format!("oboete-cursor-json-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let hooks = dir.join("hooks.json");
        let mcp = dir.join("mcp.json");
        ...
        let comments = b"{\n  // keep this\n  \"hooks\": {}\n}\n";
        std::fs::write(&hooks, comments).unwrap();
        cursor_files(&dir, &cmd, false, false).unwrap();
        assert_eq!(std::fs::read(&hooks).unwrap(), comments);
        assert_eq!(mcp_command(&mcp), Some((cmd.exe.clone(), cmd.mcp_args())));
        std::fs::remove_file(&mcp).unwrap();
        std::fs::write(&hooks, "{}\n").unwrap();
        let mcp_comments = b"{/* keep MCP comment */\"mcpServers\":{}}\n";
        std::fs::write(&mcp, mcp_comments).unwrap();
        cursor_files(&dir, &cmd, false, false).unwrap();
        assert_eq!(cursor_hooks_status(&hooks, &cmd, false), "7 hooks wired");
        assert_eq!(std::fs::read(&mcp).unwrap(), mcp_comments);
        std::fs::write(&hooks, b" \n").unwrap();
        cursor_files(&dir, &cmd, false, false).unwrap();
        assert_eq!(std::fs::read(&hooks).unwrap(), b" \n");
        std::fs::remove_dir_all(dir).unwrap();
    }

    #[test]
    fn cursor_status_and_windows_command() {
        let dir = std::env::temp_dir().join(format!("oboete-cursor-status-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let file = dir.join("hooks.json");
        let cmd = HookCommand {
            exe: r"C:\tools\oboete.exe".into(),
            home: Some(r"C:\data\oboete".into()),
        };
        assert_eq!(
            cmd.cursor_line("SessionStart", true).unwrap(),
            r#"& "C:\tools\oboete.exe" --home "C:\data\oboete" hook cursor SessionStart"#
        );
        assert_eq!(
            cursor_hooks_status(&file, &cmd, true),
            "not wired (run `oboete setup cursor`)"
        );
        let mut root = json!({"version":1,"hooks":{"sessionStart":[{"command":cmd.cursor_line("SessionStart", true).unwrap(),"timeout":10}]}});
        write_json(&file, &root).unwrap();
        assert_eq!(
            cursor_hooks_status(&file, &cmd, true),
            "partly wired: 1/7 hooks"
        );
        root["hooks"]["sessionStart"][0]["command"] =
            json!("& \"C:/old/oboete.exe\" hook cursor SessionStart");
        write_json(&file, &root).unwrap();
        assert!(cursor_hooks_status(&file, &cmd, true).contains("stale"));
        assert!(cmd.cursor_line("Stop", true).is_ok());
        let bad = HookCommand {
            exe: "C:/Program Files/oboete.exe".into(),
            home: None,
        };
        assert!(
            bad.cursor_line("Stop", true)
                .unwrap_err()
                .to_string()
                .contains("space-free")
        );
        let bad = HookCommand {
            exe: "C:/tools/$env:NAME/oboete.exe".into(),
            home: None,
        };
        assert!(bad.cursor_line("Stop", true).is_err());
        // Removal only identifies installed entries; it does not need a runnable current path.
        cursor_files(&dir, &bad, true, true).unwrap();
        assert_eq!(
            read_json_object(&file).unwrap(),
    ...
    #[test]
    fn cursor_config_precedence_and_availability() {
        let home = Path::new("/home/test");
        assert_eq!(
            cursor_dir_from(Some(Path::new("/a")), Some(Path::new("/b")), home),
            Path::new("/a")
        );
        assert_eq!(
            cursor_dir_from(None, Some(Path::new("/b")), home),
            Path::new("/b/cursor")
        );
        assert_eq!(
            cursor_dir_from(None, None, home),
            Path::new("/home/test/.cursor")
        );
        assert!(!cursor_available(
    ...
    #[cfg(unix)]
    #[test]
    fn cursor_stages_both_files_before_replacing_either() {
        use std::os::unix::fs::PermissionsExt;
        let dir = std::env::temp_dir().join(format!("oboete-cursor-stage-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let hooks = dir.join("hooks.json");
        let mcp = dir.join("mcp.json");
        ...
        for locked in [&hooks, &mcp] {
            std::fs::write(&hooks, "{}\n").unwrap();
            std::fs::write(&mcp, "{}\n").unwrap();
            std::fs::set_permissions(locked, std::fs::Permissions::from_mode(0o444)).unwrap();
            assert!(cursor_files(&dir, &cmd, false, false).is_err());
            assert_eq!(std::fs::read(&hooks).unwrap(), b"{}\n");
            assert_eq!(std::fs::read(&mcp).unwrap(), b"{}\n");
