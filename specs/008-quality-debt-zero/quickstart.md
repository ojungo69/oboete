...
# comment with the total unchanged, and a recursive scan cannot be made to work at all, because session
# transcripts, rotated backups and agent history quote whichever bundle path they were written under.
# candidate-wiring.mjs reads the four JSON files as JSON, counts the entries per file, and reads the two
# config.toml lines and the Pi loader as text with commented-out lines excluded.
