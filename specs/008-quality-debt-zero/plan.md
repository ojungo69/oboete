...
**Scale/Scope**: 745 service IDs (332 Sonar, 413 Codacy; allocation: A 302, E 46, B1 131, B2 59, B3 42, C1 27, C2 39, C3 17, C4 62, D 20) across ≈ 120 files — 707 in the 2026-09-07 export plus the 13 the live search reported on 2026-09-08 and the Claude probe file-length finding added on 2026-09-09 (14 from T030a), thirteen PR #185 findings at `ebe687dc`, and eleven matching findings with distinct IDs in Sonar's push-branch analysis (24 from T043e; 38 additions in all); 12 pull requests; ≈ 30 service-side resolutions.
...
docs/evidence/quality-debt-2026-09/*.json      # new: the frozen 745-id inventories + batch allocation
...
| E | Security-flavoured verdicts and fixes: Sonar S8786 ×13; Codacy timing 6, RegExp 7 (3 in `src/`, 2 test constants in `scripts/dco-check.test.mjs`, and the 2 the 2026-09-08 refresh re-issued for the same two `src/agents/index.ts` constructors), SSRF 1, prototype pollution 2, dynamic method 2, tainted SQL 2, SC2024 2, ci.yml 1 | session | Sonar S8786/S4036/S8707; Codacy Opengrep + ShellCheck patterns not excluded by A | 46 (34 original, 2 later Codacy findings and 5 security findings in each of the PR/push-branch analyses; later rows are reviewed individually) | A analysed |
| B1 | Mechanical rewrites, `scripts/**` | Codex | Sonar mechanical rules, file under `scripts/` | 131 (allocation; includes 3 PR and 3 push-branch IDs) | E |
| B2 | Mechanical rewrites, Codex-owned `src/**` | Codex | Sonar mechanical rules, file under `src/` and not session-owned | 59 (allocation; includes 2 PR and 2 push-branch IDs) | E |
| B3 | Mechanical rewrites, session-owned `src/**` (`injection/`, `mcp.ts`, `viewer/`, `transfer.ts`, `db/queries.ts`, `privacy/`, `fixture/replay.ts`) | session | Sonar mechanical rules, session-owned file | 42 (allocation; includes 1 PR and 1 push-branch ID) | E |
...
| F | Service-side calls and confirmations: Sonar transitions + comments, Codacy ignores with reason + comment (API, hex ids), confirmation of every planned `fixed` / `excluded` row against the analysis that dropped it, polish edits, then the inventory's final count against the final `main` SHA (amended, see above) | session | owns no ids (research R9) | 0 (records confirmations) | all, token in hand |
...
- **C3 (before F, found and decided 2026-09-07)**: Codacy's ESLint step has crashed on every analysed commit (research R10), so the 397 findings contain no ESLint finding. Decision: disable Codacy's ESLint tool for the repository (the repository's own ESLint 9 gate is the lint standard; Codacy's ESLint 8 cannot read it); the alternative, triaging the ≈ 1,100 findings the tool would surface, was declined. The feature's final count is not written while the step still appears in the analysis log. **Applied 2026-09-13**: the repository-level disable is refused with 409 while a coding standard enables the tool, so the repository follows its own standard 168669, verified equal to the organisation default 168666 except for ESLint and the Stylelint SCSS pattern (research R10 "Applied").
