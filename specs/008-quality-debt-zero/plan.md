...
| E | Security-flavoured verdicts and fixes: Sonar S8786 ×13; Codacy timing 6, RegExp 7 (3 in `src/`, 2 test constants in `scripts/dco-check.test.mjs`, and the 2 the 2026-09-08 refresh re-issued for the same two `src/agents/index.ts` constructors), SSRF 1, prototype pollution 2, dynamic method 2, tainted SQL 2, SC2024 2, ci.yml 1 | session | Sonar S8786; Codacy Opengrep + ShellCheck patterns not excluded by A | 36 (allocation: 34 in the batch, plus 2 the 2026-09-08 refresh assigned after E merged — bookkeeping rows applied in F) | A analysed |
...
| D | File-length seams: split `worker/observe.ts`, `capture.ts`, `fixture/replay.ts`, `isolated-user.mjs`, `generate-1000-events.mjs`; won't-fix rows for the other 8 (research R4/R7 tables); the 4 the 2026-09-08 refresh added (`probes/pi.mjs`, `observer/classify.ts`, `observer/llm.ts`, `setup/setup.ts`) decided the same way in T036 and implemented by whichever of the two branches owns the file | Codex on branch `008-qd-d-codex` (`observe.ts`, `capture.ts`, two scripts); session on `008-qd-d` (`replay.ts`), folded with `--ff-only` | Codacy file length not excluded by A | 17 (allocation: 5 fixed + 8 resolved + 4 decided in T036) | C1–C4 |
