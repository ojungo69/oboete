...
**Consequence**: The repository-file batch (R4) removes about 300 of the 397 Codacy findings. About 30 per-issue ignores remain (file-length "won't fix" on cohesive modules, security-flavoured findings judged not applicable, the `ci.yml` pattern hit). Those need a Codacy account API token (or the maintainer clicking each ignore). The token is a **prerequisite of batch F and of the feature's completion** (FR-002 requires confirmed dispositions for the frozen inventory); a list handed to the maintainer is an intermediate artefact, and the feature stays open until the actions have run and both services have completed the final `main` analysis, with every frozen inventory row closed or carrying a disposition confirmed against a named analysis and each service's live count recorded with its owner (issue #207). *(Amended 2026-09-14; the original clauses were "FR-002 requires 0 on the service" and "the feature stays open until the actions have run and the final `main` analysis reports 0".)*
