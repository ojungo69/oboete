...
- T034–T036 (US6, `4317d3ea`…`4b426a1c`): the owner chose "implement per the contract" on
  ...
  `test/unit/sync*.test.ts` (seven files, 179 cases; the bounds file gates its 256 MiB push and
  ...
  `/var/tmp/oboete-009-20260909.jJ5grc/us6/`) found 10 + 6 + 9 + 9 + 14 + 11 + 17 + 16 + 20 + 12 + 4 defects, fixed in
  ...
  device, is not resolved — the cycle-break is order-dependent and two devices can diverge, so it
  is tracked as a follow-up design change, not closed); round four
