...
- テストは `test/unit/sync*.test.ts` 7 ファイル 133 ケース (`test/helpers/sync.ts` 共有 fixture、
...
- レビュー: `/code-review high` 3 巡 + finder 1 角度、Codex correctness pass (`us6/secrev1..7`。security
  枠のプロンプトは OpenAI の cyber classifier に切られるので correctness 枠で出す) で 10 + 6 + 9 + 9 + 14 + 11 + 17 件。
  ...
  pass 前の row 退避 (parking) に置き換えた。semgrep 0、ponytail。契約の "Implementation notes" に round ごとの規則を記録。
