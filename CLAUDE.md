...
- **Process (owner-approved, lighter than the global default)**: spike/milestone notes in `docs/` instead of Spec Kit per PR; one review lane per PR: the Codex GitHub app reviews every push on its own, so answer each round (adopt, or reply with the reason) and merge once the run on the final head adds nothing (PR #14 took six rounds, every finding real); `cargo fmt --check`, `cargo clippy`, `cargo test` are the CI; bots: CodeQL only. GitGuardian flags the redaction test fixtures; the owner skips them as test credentials.
