...
- `oboete hook cursor` captures the seven events listed below. Session identity uses `session_id`, then `conversation_id`; repository identity uses `cwd`, then `workspace_roots[0]`, then `CURSOR_PROJECT_DIR`. Without an absolute workspace path, it skips storage instead of using the hook process's config directory. Prompt privacy, redaction, and field limits use the shared capture path. Tool output remains text; failed tools use `error_message`, assistant replies use `text`, and SessionEnd records `reason`. Cursor has SessionEnd, so its observer is not delayed.
- SessionStart and the first UserPromptSubmit after PreCompact return flat `additional_context`, capped at 9,500 UTF-16 code units. PreCompact stores only a marker. A transactional session flag preserves pending reinjection across observer cleanup and permits only one prompt to consume it. Other Cursor responses, including skips and errors, are `{}`. Every agent's stdin parser accepts a leading UTF-8 BOM. Claude-imported Cursor events remain suppressed.
- `-p` runs fire no prompt or response hook, so `sessionEnd` reads the transcript tail (`transcript_path`) and stores the turns not already stored (the n-th turn with a given prompt is new when fewer than n such prompts are stored, so repeated prompts and resumed runs keep every turn): the `<user_query>` text as the prompt and the turn's last assistant text as `Stop`. The line format comes from the bundle's agent-transcript writer (`{"role", "message": {"content": [{"type": "text"|"tool_use"}]}}`, plus `metadata` and `turn_ended` lines). Unverified: whether the transcript is flushed before `sessionEnd` fires, and whether the user text is wrapped in `<user_query>` (the local bundle does not build that wrapper; the hook takes the inner text when present and the whole text otherwise). Recovered turns are stored after the run's live tool events, so for a `-p` run with tools the observer sees the tool calls before the prompt; a one-turn print run still summarizes correctly.
- `setup cursor`, `setup all`, `--remove`, and doctor cover all seven hooks and MCP. Config directory precedence is `$CURSOR_CONFIG_DIR`, `$XDG_CONFIG_HOME/cursor`, then `~/.cursor` (installed bundle, `../cursor-config/dist/paths.js`). Claude imports still use `~/.claude/settings.json` under the home directory. Setup preserves other hooks, MCP servers, and developer fields; stages both editable files before replacement; and backs up each existing file once as `.oboete.bak`. JSONC or invalid JSON is left untouched with a manual entry, while the other file is still handled. Cursor MCP approval remains per project: `agent mcp enable oboete`, or `--approve-mcps` with `-p`.
- Linux/macOS commands use the existing POSIX quoting. Windows commands use PowerShell's `& "<exe>" [--home "<home>"] hook cursor <Event>` and reject paths disallowed by agy, plus PowerShell interpolation characters. Tests use temporary directories; no real agent configuration is changed.
- **Unverified:** all live behavior. The CLI token is expired on this PC, and the IDE is not installed. No `cursor-agent` or `agent` command was run. The installed bundle's local `-p` path does not fire `beforeSubmitPrompt`, `afterAgentResponse`, or `stop`; server-driven delivery remains unverified. Compaction, subagents, Windows PowerShell transport, IDE MCP working directories, and WSL remote behavior need live checks later.
...
2. cwd = `cwd`, falling back to `workspace_roots[0]`, then `$CURSOR_PROJECT_DIR`; skip when none identifies an absolute workspace.
...
| preCompact | PreCompact (a marker only; there is no summary) | trigger ("auto"\|"manual"), context_usage_percent, context_tokens, context_window_size, message_count, messages_to_compact, is_first_compaction, plus the common fields. Arrives through the server-driven agent-exec path. No compaction summary exists and there is no postCompact event. | Only {"user_message"}, which the user sees and the model does not. To restore context after compaction, record the marker and inject on the next beforeSubmitPrompt. |
...
6. **Headless `-p` loses prompts and replies.** The bundle shows `beforeSubmitPrompt`, `afterAgentResponse` and `stop` fired only from `ui.tsx`, the interactive TUI. In `-p` only `sessionStart`, `sessionEnd` and the tool hooks fire. A server-driven agent-exec path could still send them; I did not run it. Workaround (implemented): recover the turns from the transcript on `sessionEnd`.
