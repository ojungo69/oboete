...
3. 同期除外の判定 (決定 11): 文書はその session が触れた repo、問い合わせは検索の対象の repo (cwd の repo か `repo` 引数) で判定し、どれかが除外なら外部の埋め込み・判定・同期には出しません。対象の repo を決めない全 repo の検索 (viewer など) は、除外した repo が 1 つでもあれば外へ出しません。 同期を使っている端末では、除外の一覧が新しいことも外へ出す条件です。observe など裏で動く処理は、外へ出す前に hub から除外の一覧を取り直し (前回の取得から 5 分以内なら省く)、取り直せなければ外へ出さずに次回へ回します (hub に届かないときは要約や埋め込みの相手にも届かないことが多く、待っても失うものは少ない)。時間の限られた注入 hook は、5 分以内に取り直した一覧があるときだけ外部の埋め込みを使い、無ければ手元のモデルか全文検索だけにします。埋め込みは手元のモデルがあれば手元で作り、無ければベクトル無し (全文検索だけ) にします。tombstone だけはこの判定を通らずに送ります (§4.3)。
