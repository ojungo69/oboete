...
1. **repo のキーが絶対パスです** (`src/repo.rs` の `key()`)。WSL の `/home/jura/...`、macOS の `/Users/...`、Windows の `C:\...` で同じ repo が別物になります。これを、`.git/config` の origin URL を正規化したもの (例 `github.com/ojungo69/oboete`、git は起動しない) に変えます。正規化では URL を解析し、`https://user:password@host/...` のような認証情報 (userinfo)、query、fragment を取り除いてから、保存・表示・hash・同期に使います (取り除いたことを単体テストで確かめる)。remote が無い repo は従来どおりパスにし、本人の oboete 設定 (`oboete repo alias`) でだけ名前を付けられるようにします (repo の中のファイルには名前を決めさせない。決定 7)。既存の行も移し替えます。
...
| 同期除外の一覧 (repo キーの集合) | する (アカウント全体の設定) | `oboete sync exclude` / 除外の解除と、repo の `.oboete.toml` で見つけた `sync = false` は、どれも除外の op として hub に送り、全端末が seq の順に受け取って守る。hub も除外された repo に触れた session の中身の op を受け取らない (tombstone は受け取る)。1 台で除外した repo を、別の端末が送り直すことはない |
...
| PR-J | クラウド検索とリモート MCP | DO の FTS5、Vectorize (決定 4)、同じ RRF、`createMcpHandler`、Access、Claude アプリの custom connector 用の OAuth (workers-oauth-provider、§4.7)。リモート MCP は接続ごとに読んでよい repo を owner が許可する (`oboete remote grant <repo>`)。許可の無い repo は `search` にも `get` にも出さず、全 repo の検索は許可した repo の中だけ (悪意ある文章に操られたモデルが、関係ない repo の記憶を引き出せないように) | 評価セットでクラウドと手元の上位 10 件が 9 件以上一致する。Claude アプリから、許可した repo だけを検索できる (許可の無い repo の `get` が拒まれるテスト) |
