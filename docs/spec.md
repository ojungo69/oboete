...
22. Section 8 is settled as in RD/section-8-revised.md (2026-09-25). The owner's machines switch to the new oboete after milestone 4 (dogfood user first, old binary kept for rollback; the milestone is replaced by decision 27). The owner gives the full labelling set (about 13-19 h, in sittings of an hour or less, candidates drafted by Claude; replaced by decision 29). All eight sections are settled; next is the written spec.
23. (2026-09-25, after the sections were settled) The grok subscription is no longer used for curation. (Corrected 2026-09-26, the owner: 「grokを使わないのは要約モデルとしてです」: the exclusion is as the summarizer only. The earlier words "nor judge or digest" were Claude's, not the owner's.) To research and adopt if possible: how claude-mem uses the Claude subscription, and OpenCode's free models and OpenCode Go models as providers.
...
- **Judge trust**:
  - The trust condition, from PR-B (proposal:240), with the panel in place of the owner (owner decision 29):
    - the panel: at least three judges from different model makers, run with no tools: API providers, or subscription CLIs under their 6.5 isolation. Chosen by what answered on 2026-09-26: `claude-sonnet-5` (today's judge; Anthropic), `gpt-oss-120b` through Groq (OpenAI), and through the owner's OpenCode Go subscription `deepseek-v4-pro` (DeepSeek), `glm-5.3` (Zhipu), `kimi-k3` (Moonshot) and `qwen3.8-max` (Alibaba). NIM and Mistral answered 429 or 404 that day (Claude; overrulable). The same day the owner added the models of their grok and codex subscriptions: `grok-4.7` (xAI) and `gpt-6-astra` (OpenAI), run as the dogfood user with every tool denied (grok) or under the curator permission profile (codex), and calibrated in run 3 of the same pairs (docs/spike/cli-judges.md). Eight judges from seven makers: each judge has seven others, whose majority can tie; a tie is left out and counted, as below;
    - about 50 calibration pairs, each graded by every panel judge;
    - a judge passes when its binary relevance κ ≥ 0.4 against the majority of the other panel judges (a pair they tie on is left out and counted), and the panel's own Fleiss κ ≥ 0.4, until 10 or more system configurations exist;
    - after that, Kendall τ ≥ 0.85 between the judge's system order and the order from the other panel judges' majority (the judge's own grades never count toward its reference; pairs they tie on are left out), measured on 20-30 questions whose pooled top 50 of every compared configuration is fully judged by the other panel judges, never on scattered pairs (issue #30 row 22; Claude; overrulable).

    Until one of these passes, the judge decides nothing, and a panel whose own Fleiss κ is below 0.4 labels nothing.
  - What the panel cannot show: judges that share a bias agree on the same wrong grade. So the owner's labels (below) are the only human reference, and on them the judge's and the panel's agreement with the owner is reported next to the panel result.
  - B3 (docs/pr-b.md:9) is milestone 1's first gate.
  - D2's test result (hybrid nDCG@10 0.545) was judged by claude-sonnet-5 before the trust condition was measured (eval/runs-test/report-test.txt:1; docs/pr-d.md:173). So that result, and the default it set, are provisional. If B3 fails, D2's test pool is judged again, by a judge that passes (never by the majority of a panel that failed), before M1 uses it as the baseline.
  ...
  - One owner does all the owner labelling (RD/constraints-synthesis.md:101). At least a week after labelling, the owner labels a blind random 20 of their earlier items again. The owner's agreement with their own earlier labels is reported next to the judge's agreement (Claude; overrulable).
- **Where labels come from**:
  - The owner labels what only the owner knows, in plain Japanese, with their own prompt shown (owner decision 29):
    - whether a drafted decision or preference is theirs;
    - whether a later decision overturned an earlier one (overturned and control pairs);
    - the claims the curator marked `decided`.
    - An item the owner marks "判断できない" goes to the panel and is counted apart.
  - The panel labels technical relevance: calibration pairs, search pools, no-answer and false-premise questions, per-kind labels, answer keys and spot-checks.
...
1. **Freeze and label** (issue50 §8 row 1, RD/issue50.md:159):
   ...
   - Labels: dev labels first, because the curator spike needs them; test labels before milestone 3's deciding run.
     - Claude drafts every candidate. The owner confirms, rejects or marks "判断できない" only on items about their own decisions, each in plain Japanese with their own prompt (owner decision 29): about 4-5 h in total, in sittings of an hour or less (Claude's estimate). The panel (8.1) grades the rest.
     - Before the curator spike:
       - the panel: B3's 50 calibration pairs;
       - the owner: about 50 dev decisions; 20 dev overturned pairs with 20 control pairs; after a week, a blind 20 of these again.
     - Before milestone 3's deciding run:
       - the owner: 50 test overturned pairs with 50 control pairs (at least 20 across sessions); 100 real decisions in the held-out transcripts;
       - the panel: yes/no on about 100 no-answer and 100 false-premise questions (§4.6, RD/sections-1-4.md:44).
     - During milestones 3-4:
       - the owner: 100 claims the curator marked `decided`;
       - the panel: about 100 per-kind labels (MUST-M21); M5's 20% spot-check; answer keys for M6's 40 questions.
...
| A68 | Kendall τ is measured on 20-30 questions fully judged by the panel, never on scattered pairs; the panel's makers and models | 8.1 | How judge trust is measured (issue #30 row 22) |
| A69 | The owner labels a blind 20 of their earlier items again after a week | 8.1 | An extra labelling task |
...
5. How Kendall τ is measured (row 30-22): on 20-30 questions whose pooled top 50 of every compared configuration is fully judged by the panel (owner decision 29), never on scattered pairs (8.1).
