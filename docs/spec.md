...
- **Judge trust**:
  - The trust condition, from PR-B (proposal:240), with the panel in place of the owner (owner decision 29):
    - the panel: at least three judges from different model makers, run with no tools: API providers, or subscription CLIs under their 6.5 isolation. Candidates: `claude-sonnet-5` (today's judge), `gpt-oss-120b` (Groq), Nemotron (NIM), GLM (OpenCode Go), Mistral (Claude; overrulable);
