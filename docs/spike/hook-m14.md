...
- Windows, GNU build: built on WSL with `cargo zigbuild --release --target x86_64-pc-windows-gnu` (zig 0.16 from the `ziglang` package), run from `C:\Users\jura` (NTFS, Defender scanning included, as a Windows user gets it). **This is not the release artifact**: Windows native ships as `x86_64-pc-windows-msvc`, and this PC has no MSVC build tools. Process start and the bundled SQLite are toolchain-built and inside the timed interval, so the MSVC binary is unmeasured; milestone 2 measures it. NTFS and Defender, which dominate the tails here, are the same for both builds.
...
| Windows, GNU build (NTFS) | 1 KB | off | off | 10.1 | 11.5 | 12.1 | 12.4 |
| Windows, GNU build (NTFS) | 1 KB | full | off | 12.2 | 13.5 | 14.1 | 67.0 |
| Windows, GNU build (NTFS) | 1 KB | full | level 3 | 12.6 | 14.9 | 22.1 | 67.5 |
| Windows, GNU build (NTFS) | 64 KB | off | off | 12.2 | 14.0 | 14.9 | 16.5 |
| Windows, GNU build (NTFS) | 64 KB | full | off | 18.3 | 20.2 | 21.6 | 150.5 |
| Windows, GNU build (NTFS) | 64 KB | full | level 3 | 18.6 | 27.7 | 34.2 | 100.1 |
| Windows, GNU build (NTFS) | 256 KB | off | off | 24.8 | 34.1 | 140.9 | 545.0 |
| Windows, GNU build (NTFS) | 256 KB | full | off | 34.7 | 41.4 | 93.1 | 142.6 |
| Windows, GNU build (NTFS) | 256 KB | full | level 3 | 28.6 | 34.7 | 55.6 | 116.2 |
...
Control on the iMac, `synchronous=FULL` without `fullfsync` (`HOOK_M14_NO_FULLFSYNC=1`, same harness otherwise; asked for in the PR #68 review):

| Machine | Output | Redaction | zstd | p50 ms | p95 ms | p99 ms | max ms |
|---|---|---|---|---|---|---|---|
| M1 iMac (APFS, FULL only) | 1 KB | off | off | 2.6 | 3.2 | 5.2 | 10.1 |
| M1 iMac (APFS, FULL only) | 1 KB | full | off | 3.9 | 4.1 | 4.3 | 4.8 |
| M1 iMac (APFS, FULL only) | 1 KB | full | level 3 | 3.9 | 4.1 | 4.2 | 4.3 |
| M1 iMac (APFS, FULL only) | 64 KB | off | off | 3.0 | 3.1 | 3.1 | 3.2 |
| M1 iMac (APFS, FULL only) | 64 KB | full | off | 7.2 | 7.3 | 7.4 | 8.9 |
| M1 iMac (APFS, FULL only) | 64 KB | full | level 3 | 7.2 | 7.2 | 7.4 | 20.0 |
| M1 iMac (APFS, FULL only) | 256 KB | off | off | 3.9 | 4.0 | 4.1 | 5.4 |
| M1 iMac (APFS, FULL only) | 256 KB | full | off | 13.0 | 13.2 | 13.3 | 14.3 |
| M1 iMac (APFS, FULL only) | 256 KB | full | level 3 | 12.8 | 12.9 | 13.0 | 13.0 |

## What it says for milestone 2

- **The slowest machine depends on the write size.** Up to 64 KB it is the iMac, where `F_FULLFSYNC` sets the floor: with `fullfsync`, even a 1 KB write without redaction takes 23 ms p95, and without it (the control) 3.2 ms; redaction and size add only 1-4 ms. So 20 ms p95 cannot hold on macOS while every hook write is an `F_FULLFSYNC`. Milestone 2 has to choose between: keeping `fullfsync` and setting the line from it (about 25-27 ms p95), dropping `fullfsync` to `synchronous=FULL` alone on macOS (plain fsync, weaker on Apple SSDs; the control stays within 3-13 ms p95 at every size), or not syncing in the hook at all and letting the worker sync a spool. This is a durability decision, not a tuning one.
- **At 256 KB Windows (GNU build) is the slowest machine** (41.4 ms p95 with full redaction, against 27.0 ms on the iMac). 64 KB with full redaction: 20.2 ms p95 (27.7 ms with zstd). 256 KB: 34-41 ms p95, with tails past 100 ms (Defender). Without redaction the 256 KB write is still 34 ms p95, so size, not redaction, is Windows' cost.
