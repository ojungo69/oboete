...
> 2026-09-26: 設計の正本は `docs/spec.md` (再設計、owner と 2026-09-25/26 に確定) に移った。この文書は owner の元の希望 (作りたいもの 12 項目)、M0 結果、今のコード (M1〜) の決定を残す。以下の「方針」2b・3・4・8 とマイルストーンは spec が置き換える。
...
3. **データ = SQLite 1 ファイル** `~/.oboete/oboete.db`: sessions / events(生。削除しない: 2026-09-25 から要約後も残し、`sessions.observed_event_id` で要約済みの位置を持つ。旧コードは要約直後に消していた)/ observations(claude-mem の型)/ summaries / prompts / fts(trigram + CJK bigram)/ vec(sqlite-vec + fastembed multilingual-e5-small)/ injections / provider_calls。約 9 表。work item・共有承認・移行記録は作らない。
4. **要約 = provider chain + fallback**。設定は順序付きリスト。既定: Groq free (120b → 20b) → OpenRouter free → Mistral free → NIM → OpenCode Go → codex → claude → (有料 API は明示設定時のみ末尾)。この順番は 2026-09-27 に owner が決めた: 無料を先に、次に定額の OpenCode Go、最後にコーディング用のサブスク。NIM は owner の実際の呼び出しで一度も成功していない (9/22-9/26、0 成功・16 失敗) ので無料の最後 (それまでは 2026-09-26 の Groq → NIM → OpenCode Go → codex → claude → OpenRouter → Mistral、その前は claude → OpenRouter → NIM → Mistral → codex)。OpenCode Go を NIM の次に置いたのは Claude の判断 (キーで呼ぶ定額サブスクで、CLI のサブスク枠より先に使う)。grok は 2026-09-25 に既定から外した: owner の決定で grok のサブスク枠は要約 (と判定・digest) に使わない (明示設定すれば使える)。サブスク CLI は安いモデルで呼ぶ (2026-09-26、owner 指摘「claude-mem でサブスクを使う時は haiku」): claude は `haiku`、codex は `gpt-6-luna` (reasoning low)。要約はあなたが作業に使う枠を削るため。provider の答えは chain の中で schema の形 (型・必須キー・配列の中身) を確かめ、違えば次の provider へ (strict schema の無い provider が別の形の JSON を返しても窓を落とさない)。OpenCode Go (owner 契約済み) は API キー型のサブスクで、キー `~/OPENCODE_API_KEY.md` があれば既定の chain で使われる: `kind = "openai"`、`base_url = "https://opencode.ai/zen/go/v1"`、`model = "glm-5.3-flash"` (学習に使われず保存 0 日)、`headers = { "x-opencode-session" = "oboete" }` (新しいキー `oc_sk_…` はこのヘッダーが無いと 400 `MissingSessionID`、2026-09-26 に確認)。 glm-5.3-flash は strict な `json_schema` を指定しても答えを ```json の囲みで包むことがある (2026-09-26、4 回中 3 回) ので、`openai` 型の答えは囲みを外してから読む。推論してから答えるので 1 回 13〜35 秒。`[[providers]]` の `headers` は provider ごとの追加ヘッダーで、キーは入れない (キーは `key_file`)。OpenCode Zen の無料モデルは OpenCode 以外からの呼び出しを 403 で断るので使わない (2026-09-25 の probe)。agy は 2026-09-25 に既定から外した: headless の agy は道具を切るスイッチが無く、利用者の設定 (道具の許可・plugin) をそのまま引き継ぐため、信頼できない記録を読む要約役には置かない (明示設定すれば使えるが、`--dangerously-skip-permissions` は付けない)。各 provider の性格: Groq は 1,000 回/日で最安定、サブスク枠は owner が「腐っている」、OpenRouter free は $10 未購入だと 50 回/日、NIM は上限非公開。429/5xx/timeout → その provider を cooldown して次へ。JSON schema 検証失敗 → 次へ。全滅 → pending のまま次回。生イベントは要約後も保持。**provider ごとに日次予算**(fallback 自体が暴走しないため。TS 版 #352 の教訓を仕様として引き継ぐ)。Gemini free は opt-in。codex / agy は system prompt フラグが無いので指示は user prompt に同梱。
