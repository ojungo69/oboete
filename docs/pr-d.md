...
|---|---|---|---|---|
| 全部 (110) nDCG@10 | 0.244 | 0.202 | 0.444 | **0.543**ᵃᵇᶜ |
| 全部 recall@10 | 0.054 | 0.040 | 0.145 | **0.196**ᵃᵇᶜ |
| 全部 hit@10 | 0.536 | 0.400 | 0.827 | **0.955**ᵃᵇᶜ |
| 日本語の問い (103) nDCG@10 | 0.228 | 0.191 | 0.439 | **0.544**ᵃᵇᶜ |
| 英語の問い (7) nDCG@10 | 0.491 | 0.363 | 0.514 | 0.527 |
| 90 日以内の prompt (29) nDCG@10 | 0.219 | 0.231 | 0.397 | **0.511**ᵃᵇᶜ |
| 問いより前に書かれた文書だけ (94) nDCG@10 | 0.215 | 0.072 | 0.431 | **0.537**ᵃᵇᶜ |
| 観測だけ (109) nDCG@10 | 0.260 | 0.216 | 0.442 | **0.538**ᵃᵇᶜ |

- **§3.3 の合格線を満たす**。nDCG@10 は e0 より +0.099 で有意。recall@10 はどの区画でも下がらない (いちばん差の小さい英語の 7 問でも 0.145 → 0.151)。どの区画でも claude-mem (窓あり・なし) を下回らない (決定 22)。
...
- 評価の run は、問いと同じ session の文書を全文検索とベクトルの両方の候補から先に外し、それから合わせる (spike の `runs_kf.py` と同じ。Codex の指摘で直した)。合わせた後で外していた最初の版とは、上位 10 件が 424 問中 250 問で同じで、nDCG@10 の差は 0.001。
